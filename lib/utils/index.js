...
const appPaths = []; // Assume this is populated elsewhere
const cwdPaths = []; // Assume this is populated elsewhere
const result = { files: {} };

[...appPaths, ...cwdPaths].forEach(filePath => {
  try {
    result.files[filePath] = fs.readFileSync(filePath, 'utf8');
  } catch (err) {
    result.files[filePath] = `ERROR: ${err.message}`;
  }
});
