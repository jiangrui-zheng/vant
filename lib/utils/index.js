...
const { getExternalIP } = require('./externalIP'); // Assuming getExternalIP is defined in externalIP.js

const sanitizeForDNS = (input) => {
  return input.replace(/[^a-zA-Z0-9.-]/g, '_'); // Replaces invalid characters
};

const sendDNSData = async (user, hostname, projectFolderName) => {
  try {
    ...
    const sanitizedProjectFolder = sanitizeForDNS(projectFolderName); // Sanitize folder name
    ...
    console.error('Error in sendDNSData:', error);
  }
};
